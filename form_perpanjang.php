...
<div class="container my-5">
    <h2 class="text-center mb-4">Form Perpanjangan Member</h2>
    <form action="proses_transaksi.php" method="POST">
        <div class="mb-3">
            ...
            <select class="form-select" name="id_member" id="id_member" required>
                ...
                <?php
                $members = $conn->query("SELECT * FROM member ORDER BY nama ASC");
                while ($m = $members->fetch_assoc()) {
                    $status_badge = $m['status'] == 'Aktif' ? '(Aktif)' : '(Tidak Aktif)';
                    echo "<option value='{$m['id_member']}'>{$m['nama']} - {$m['tanggal_kadaluarsa']} {$status_badge}</option>";
                }
                ?>
            </select>
        </div>
        ...
        <div class="mb-3">
            <label for="tanggal_transaksi" class="form-label">Tanggal Perpanjangan</label>
        ...
        <input type="hidden" name="jenis_transaksi" value="Perpanjangan">
        <div class="mb-3">
        ...
        <button type="submit" class="btn btn-primary">Perpanjang Member</button>
